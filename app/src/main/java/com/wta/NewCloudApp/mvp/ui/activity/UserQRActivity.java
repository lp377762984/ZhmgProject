...
public class UserQRActivity extends BaseLoadingActivity<UserQRPresenter> implements UserQRContract.View, MoneyBar.CallBack, View.OnClickListener {
    ...
    private void setImQr(Drawable drawable) {
        ...
        if (drawable != null) {
            ...
            int wah2 = (int) ScreenDpiUtils.dp2px(this, 50);
            imageView.setLayoutParams(new ViewGroup.LayoutParams(wah2, wah2));
            imageView.setCornerRadius(10);
            imageView.setBorderColor(Color.RED);
            imageView.setBorderWidth(getResources().getDimension(R.dimen.qr_bolder_wide));
            imageView.setImageDrawable(drawable);
            bitmap = BitmapUtils.convertViewToBitmap(imageView);
        }
    ...
    private void saveBitmap() {
        ...
        try {
            ...
            bos.flush();
            bos.close();
            ArmsUtils.makeText(App.getInstance(), "以保存到" + file.getAbsolutePath());
        } catch (IOException e) {
            e.printStackTrace();
